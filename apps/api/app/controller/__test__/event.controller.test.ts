...
describe('getOne', () => {
  ...
  test('should return a json with an event when event is found', async () => {
    const res = mockResponse()
    Event.getEventById = vi.fn().mockResolvedValue({ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 })
    //@ts-ignore
    await getOne(req, res)
    expect(res.json).toBeCalledWith({ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 })
  });
...
describe('updateOne', () => {
  ...
  test('should return a json with success true when event is updated', async () => {
    const res = mockResponse()
    const req = mockRequest
    Event.findByPk = vi.fn().mockResolvedValue({ organizer_id: 2 })
    Event.update = vi.fn().mockResolvedValue(true)
  ...
  test('should throw an AuthorizationError if event is not found', async () => {
    ...
    await expect(async () => updateOne(req, res)).rejects.toThrowError('Operation not allowed')
  })
  test('should call event update with the correct params', async () => {
    ...
    await updateOne(req, res)
    expect(Event.update).toBeCalledWith(req.body.event_id, { location: 'test', duration: 90 })
  })
});
describe('deleteOne', () => {
  afterEach(() => {
  ...
  test('should return a json with success true when event has been deleted', async () => {
    const res = mockResponse()
    const req = mockRequest
    Event.findByPk = vi.fn().mockResolvedValue({ organizer_id: 2 })
    Event.delete = vi.fn().mockResolvedValue(true)
  ...
  test('should throw an Error No event if not event is found', async () => {
    ...
    Event.delete = vi.fn().mockResolvedValue(true)
    //@ts-ignore
    await expect(async () => deleteOne(req, res)).rejects.toThrowError('No event')
  })
  test('should throw an AuthorizationError if organizer is not equal to profileId', async () => {
    const res = mockResponse()
    const req = mockRequest
    Event.findByPk = vi.fn().mockResolvedValue({ organier_id: 3 })
    Event.delete = vi.fn().mockResolvedValue(true)
    //@ts-ignore
    await expect(async () => deleteOne(req, res)).rejects.toThrowError('Operation not allowed')
  })
...
describe('getAllByUser', () => {
  ...
  test('should return a json with events when events are found', async () => {
    const res = mockResponse()
    Event.getEventByUserId = vi.fn().mockResolvedValue([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
    //@ts-ignore
    await getAllByUser(req, res)
    expect(res.json).toBeCalledWith([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
  })
...
describe('getOrganizerEvents', () => {
  ...
  test('should return a json with events when events are found', async () => {
    const res = mockResponse()
    Event.getOrganizerEvents = vi.fn().mockResolvedValue([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
    //@ts-ignore
    await getOrganizerEvents(req, res)
    expect(res.json).toBeCalledWith([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
  })
...
describe('getPasts', () => {
  ...
  test('should return a json with events when events are found', async () => {
    const res = mockResponse()
    Event.getPastEvents = vi.fn().mockResolvedValue([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
    //@ts-ignore
    await getPasts(req, res)
    expect(res.json).toBeCalledWith([{ id: 1, location: 'test', organizer_id: 2, date: '2021-10-10', duration: 60 }])
  })
