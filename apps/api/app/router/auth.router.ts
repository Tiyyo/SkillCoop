...
router.route('/register').post(validate(registerSchema, canals.body), factory(register));

router.route('/login').post(validate(loginSchema, canals.body), factory(signin));

router.route('/refresh').get(tokenHandler.validate('refresh'), factory(refresh));

router.route('/email').post(validate(emailSchema, canals.body), factory(resendEmail));
...
router.route('/reset-password').get(verifyResetPasswordToken).post(factory(resetPassword));
