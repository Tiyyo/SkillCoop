...
export default {
  createToken: function (expireTime: string, key: string, payload: Record<string, any>) {
    const token = sign(
      { data: payload }, key, {
  ...
  createPairAuthToken: function (payload: Record<string, any>) {
    const accessToken = this.createToken('15m', process.env.JWT_TOKEN_KEY as string, payload)
    const refreshToken = this.createToken('7d', process.env.JWT_REFRESH_TOKEN_KEY as string, payload)
  ...
  validateToken: (token: string, tokenType: 'email' | 'refresh' | 'access') => {
    ...
    switch (tokenType) {
      case 'email':
        return payload = (this as any).default.verifyTokenAndGetData(token, process.env.JWT_EMAIL_TOKEN_KEY as string)
      case 'refresh':
        return payload = (this as any).default.verifyTokenAndGetData(token, process.env.JWT_REFRESH_TOKEN_KEY as string)
      case 'access':
        const payloadAccess = (this as any).default.verifyTokenAndGetData(token, process.env.JWT_TOKEN_KEY as string)
  ...
  getToken: (request: Request, type: 'email' | 'refresh' | 'access'): string | null => {
    ...
    switch (type) {
      case 'email':
        return token = (this as any).default.getEmailToken(request)
      case 'refresh':
        return token = (this as any).default.getRefreshToken(request)
      case 'access':
        return token = (this as any).default.getAccessToken(request)
      default:
        return null
    }
  },
