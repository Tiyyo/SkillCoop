...
function useInfinite({ queryKey, queryFn, argsFn, elementPerPage }: IUseInfinite) {
  const { data, isError, isLoading, isFetching, hasNextPage, fetchNextPage } = useInfiniteQuery({
    queryKey: [queryKey], queryFn: ({ pageParam = 1 }) => {
      if (!argsFn) return
