...
import { isFalsyExceptZero } from 'src/infrastructure/utils/falsy-except-zero';
...
export class EventStatusEvaluator {
  ...
  async evaluate(
    ...
    if (
      isFalsyExceptZero(requiredParticipant) ||
      isFalsyExceptZero(this.confirmedParticipants)
    ) {
      return 'notFound';
    }
