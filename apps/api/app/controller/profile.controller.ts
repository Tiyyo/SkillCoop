...
export default {
    ...
    async getOne(req: Request, res: Response) {
        ...
        const profile = await cacheOrGetCacheData(`profile${id}`, async () => {
            try {
    ...
    async createImage(req: Request, res: Response) {
        ...
        await redisClient.del([`profile${profile_id}`, 'profiles'], (err, reply) => {
            if (err) throw new ServerError('Could not delete cache')
            logger.debug(`delete cache ${reply}`)
        })
    ...
    async updateImage(req: Request, res: Response) {
        ...
        const newImage = await Image.update(profile_id, { url: key, key: key, size: 100 })
