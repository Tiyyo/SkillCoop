...
describe('useRigthCondition', () => {
  ...
  test('should call random condition if participant is equal of length [ids]', () => {
    const mockConfig = {
  ...
  test('should call random condition ifZero if player have 0 as rating', () => {
    ...
    expect(condition.ifZero).toHaveBeenCalledWith(mockConfig.team1.length, mockConfig.team2.length)
  })
  test('should call regular condition', () => {
    const mockConfig = {
      team1: [{ gb_rating: 2, profile_id: 2 }, { gb_rating: 23, profile_id: 3 }],
    ...
    expect(condition.regular).toHaveBeenCalledWith(mockValueTeam1, mockValueTeam2)
...
describe('deleteFromArrayAfterPush', () => {
  test("should delete the index from the array"), () => {
    expect(typeof deleteFromArrayAfterPush([1, 2, 3], [1, 2, 3], 0)).toBeTypeOf('object')
...
describe('getPlayerObject', () => {
  test("should return an object with the profile_id and gb_rating"), () => {
    expect(typeof getPlayerObject(0, [1, 2, 3], [1, 2, 3])).toBeTypeOf("object")
