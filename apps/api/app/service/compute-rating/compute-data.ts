...
export class ComputeRating {
  ...
  constructor(userEvaluationsAndBonus: UserEvaluationsBonus, profileId: number) {
    ...
    this.nbEvalReceived = this.receivedEval ? this.receivedEval.nb_eval_received : 0
  ...
  applyMvpBonus(rating: number) {
    ...
    const gbRatingMvpApplied = ((rating * (totalEvals)) + this.nb_mvp * this.VALUE_BONUS) / (totalEvals + this.nb_mvp)
  ...
  computeAvgSkillValue(skill: Partial<Skills> | string) {
    const args = {
      ...
      receivedEval: this.receivedEval ? this.receivedEval[`${skill}` as keyof Skills] : 0,
  ...
  constructAvgSkills(): Score {
    return this.skillNames.reduce((acc, currSkillName) => {
      ...
      return {
        ...acc, [skillName]: this.computeAvgSkillValue(currSkillName)
      }
    }, {}) as Score
  }
  getNumerator(args: ArgsSkillCompute) {
    return this.productNumerator(args.ownEval, this.WEIGHT_OWN_EVAL) + this.productNumerator(args.receivedEval) + this.productNumerator(this.VALUE_BONUS, args.nbBonus)
  }
